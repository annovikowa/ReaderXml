...
namespace ConverterXlsxLibrary.Fillers.KPT
{
    ...
    public class ZoneFiller : IFiller<Zone>
    {
                            ...
                            case "PermittedUse":
                                {
                                    ...
                                    model.AdditionalInformation += separator + ExtractingAdditionalInformation(reader.ReadSubtree(), xsdDictionaries, model.Error);
        ...
        private string ExtractingAdditionalInformation(XmlReader reader, XsdClassifiers dictionary, string error)
        {
            var type = "";
                    ...
                    if (reader.NodeType == XmlNodeType.Element)
                    {
                        switch (reader.LocalName)
                        {
                            ...
                            case "PermitedAncillary":
                                {
                                    ancillaries.Add(ExtractingAdditionalInformation(reader.ReadSubtree(), dictionary, error));
                                    break;
                                }
                        }
                    }
